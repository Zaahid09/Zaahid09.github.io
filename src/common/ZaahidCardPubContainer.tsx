...
interface ZaahidCardArticleContainerProps {
  img: string;
  title: string;
  text: string;
  downloadLink: string;
...
export function ZaahidCardArticleContainer(
  props: ZaahidCardArticleContainerProps,
) {
  ...
  return (
    <div className="col-12 col-md-4">
      <div
        className={`container shadow-sm Zaahid-container bg-${theme} text-${theme === "light" ? "dark" : "white"}`}
      >
        <div className="row">
          <div style={{ width: "100%", height: "200px" }}>
            <img
              className="Zaahid-img-responsive"
              src={props.img}
              alt={props.title}
            />
          </div>
          ...
          <div
            className={`card bg-${theme} text-${theme === "light" ? "dark" : "white"}`}
          >
            <div className="card-body">
              <h5 className="card-title">{props.title}</h5>
              <div className="mb-2">
                {props.authors &&
                  props.authors.map((category, index) => (
                    <span
                      key={index}
              ...
              <div className="read-article mt-4">
                <a
                  className="d-flex justify-content-between align-items-center text-decoration-none fw-bold"
                  target="_blank"
